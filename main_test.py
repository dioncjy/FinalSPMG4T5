...
import os
import sys
from backend.staff import staff_test_db
from backend.hr import hr_test_db
...
def create_app():
    ...
    def get_applicants_by_listing(listing_id):
        role_applications_data = hr_test_db.getApplicantsByListing(listing_id)
    ...
    def get_specific_applicant(listing_id, role_name, staff_id):
        specific_applicant_data = hr_test_db.getSpecificApplicant(listing_id, role_name, staff_id)
    ...
    def get_staff_details(staff_id):
        staff_details_data = hr_test_db.getStaffDetails(staff_id)
    ...
    def get_all_role_skills_details():
        role_skills_data = hr_test_db.getAllRoleSkills()
    ...
    def insert_application(staff_id, comments, role_name, listing_id):
        insert_application_data = hr_test_db.insertApplication(staff_id, comments, role_name, listing_id)
    ...
    def add_role(role_name, dpt, closing_date, opening_date, reporting_manager):
        addrole_data = hr_test_db.addRole(role_name, dpt, closing_date, opening_date, reporting_manager)
        return addrole_data

    # Get all staff skill
    @app.route('/staff_skill', methods=['GET'])
    def get_all_staff_skill():
        all_staff_skills = hr_test_db.getAllStaffSkills()
    ...
    def auto_role_details(role_name):
        auto_role_details = hr_test_db.autoPopulateRoleDetails(role_name)
    ...
    def editRL(listing_id, role_name, dpt, closing_date, opening_date, reporting_manager):
        editRL = hr_test_db.editRole(listing_id, role_name, dpt, closing_date, opening_date, reporting_manager)
        return editRL
    ...
    def get_role_listing_details(role_name):
        role_data = staff_test_db.getRoleDetails(role_name)
        return role_data
